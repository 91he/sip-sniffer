...
void scan_init(void) {
    ...
    if (nids_params.scan_num_hosts > 0) {
        gettimeofday(&tv, 0);
        time0 = tv.tv_sec;
        hashhost = (struct host **)calloc(nids_params.scan_num_hosts,
                                          sizeof(struct host *));
        if (!hashhost)
...
static int scan_hash(int addr) {
    return ((addr % 65536) ^ (addr >> 16)) % (nids_params.scan_num_hosts);
}

void detect_scan(struct ip *iph) {
    int i;
    struct tcphdr *th;
    int hash;
    ...
    if (!this_host) {
        if (i == 10)
            ...
            this_host = (struct host *)malloc(sizeof(struct host) +
                                              (nids_params.scan_num_ports + 1) *
                                                  sizeof(struct scan));
            if (!this_host)
                nids_params.no_mem("detect_scan");
            this_host->packets =
                (struct scan *)(((char *)this_host) + sizeof(struct host));
            if (hashhost[hash]) {
    ...
    this_host->packets[this_host->n_packets].flags =
        *((unsigned char *)(th) + 13);
    this_host->n_packets++;
