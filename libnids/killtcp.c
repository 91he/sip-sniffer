...
void nids_killtcp_seq(struct tcp_stream *a_tcp, int seqoff) {
    ...
    libnet_build_tcp(a_tcp->addr.source, a_tcp->addr.dest,
                     a_tcp->client.first_data_seq + a_tcp->server.count +
    ...
    libnet_build_tcp(a_tcp->addr.dest, a_tcp->addr.source,
                     a_tcp->server.first_data_seq + a_tcp->client.count +
...
void nids_killtcp_seq(struct tcp_stream *a_tcp, int seqoff) {
    ...
    tcp_tag = libnet_build_tcp(
        ...
        a_tcp->client.first_data_seq + a_tcp->server.count +
            a_tcp->server.urg_count + (seqoff ? (a_tcp->server.window / 2) : 0),
        0, 0x4, 32000, 0, 0, LIBNET_TCP_H, NULL, 0, l, tcp_tag);
    ip_tag = libnet_build_ipv4(LIBNET_TCP_H + LIBNET_IPV4_H, 0, 12345, 0, 64,
                               IPPROTO_TCP, 0, a_tcp->addr.saddr,
                               a_tcp->addr.daddr, 0, 0, l, ip_tag);
    libnet_write(l);
    tcp_tag = libnet_build_tcp(
        a_tcp->addr.dest, a_tcp->addr.source,
        a_tcp->server.first_data_seq + a_tcp->client.count +
            a_tcp->client.urg_count + (seqoff ? (a_tcp->client.window / 2) : 0),
        0, 0x4, 32000, 0, 0, LIBNET_TCP_H, NULL, 0, l, tcp_tag);
    ip_tag = libnet_build_ipv4(LIBNET_TCP_H + LIBNET_IPV4_H, 0, 12345, 0, 64,
