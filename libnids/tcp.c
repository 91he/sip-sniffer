...
static void add_tcp_closing_timeout(struct tcp_stream *a_tcp) {
    ...
    newto->prev = 0;
    for (newto->next = to = nids_tcp_timeouts; to;
...
static void notify(struct tcp_stream *a_tcp, struct half_stream *rcv) {
    ...
    if (rcv->collect) {
        ...
        } while (nids_params.one_loop_less && a_tcp->read > 0 &&
                 rcv->count_new);
...
static void tcp_queue(struct tcp_stream *a_tcp, struct tcphdr *this_tcphdr,
    ...
    if (!after(this_seq, EXP_SEQ)) {
        if (after(this_seq + datalen + (this_tcphdr->th_flags & TH_FIN),
                  EXP_SEQ)) {
...
struct tcp_stream *find_stream(struct tcphdr *this_tcphdr,
...
//ECN
    ...
    if ((this_tcphdr->th_flags & TH_SYN)) {
        if (from_client) {
            ...
            if (nids_params.tcp_flow_timeout > 0 &&
                (a_tcp->ts + nids_params.tcp_flow_timeout <
        ...
        if (a_tcp->client.wscale_on) {
            a_tcp->server.wscale_on =
                get_wscale(this_tcphdr, &a_tcp->server.wscale);
    ...
    if (!(!datalen && ntohl(this_tcphdr->th_seq) == rcv->ack_seq) &&
        (!before(ntohl(this_tcphdr->th_seq),
    ...
    if (datalen + (this_tcphdr->th_flags & TH_FIN) > 0)
        tcp_queue(a_tcp, this_tcphdr, snd, rcv,
                  (char *)(this_tcphdr) + 4 * this_tcphdr->th_off, datalen,
                  skblen);
...
void nids_register_tcp(void(*x)) { register_callback(&tcp_procs, x); }

void nids_unregister_tcp(void(*x)) { unregister_callback(&tcp_procs, x); }
...
int tcp_init(int size) {
    ...
    streams_pool = (struct tcp_stream *)malloc((max_stream + 1) *
