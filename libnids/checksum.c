...
inline u_short ip_fast_csum(u_char *iph, u_int ihl) {
    ...
    __asm__ __volatile__(
        ...
        "2:						\n"
        /*
          Since the input registers which are loaded with iph and
          ipl are modified, we must also specify them as outputs,
...
static inline u_int csum_fold(u_int sum) {
    __asm__("	addl %1, %0		\n"
            "	adcl $0xffff, %0	\n"
            : "=r"(sum)
...
*/
static inline u_short csum_tcpudp_magic(u_int saddr, u_int daddr, u_short len,
...
/*
  ...
  icmp.c
*/
inline u_short ip_compute_csum(u_char *buff, int len) {
    return (csum_fold(csum_partial(buff, len, 0)));
}

inline u_short my_tcp_check(struct tcphdr *th, int len, u_int saddr,
                            u_int daddr) {
    if (dontchksum(saddr))
        return 0;
...
inline u_short my_udp_check(void *u, int len, u_int saddr, u_int daddr) {
    if (dontchksum(saddr))
        return 0;
