...
static void nids_syslog(int type, int errnum, struct ip *iph, void *data) {
    ...
    case NIDS_WARN_TCP:
        ...
        if (errnum != NIDS_WARN_TCP_HDR)
            syslog(nids_params.syslog_level, "%s,from %s:%hu to  %s:%hu\n",
    ...
    case NIDS_WARN_SCAN:
        ...
        sprintf(buf,
                "Scan from %s. Scanned ports: ", int_ntoa(this_host->addr));
...
#error DLT_PRISM_HEADER is defined, but DLT_IEEE802_11 is not ???
    ...
    case DLT_IEEE802_11_RADIO:
        ...
        if (!linkoffset_tweaked_by_prism_code) {
            nids_linkoffset =
                EXTRACT_LE_16BITS(data + 2); // skip radiotap header
    ...
    case DLT_IEEE802_11:
         ...
         */
        if (!linkoffset_tweaked_by_prism_code &&
            !linkoffset_tweaked_by_radio_code)
            nids_linkoffset = 0;
 ...
 */
    ...
    if ((unsigned long)(data + nids_linkoffset) & 0x3) {
        ...
        memcpy(data_aligned, data + nids_linkoffset,
               hdr->caplen - nids_linkoffset);
    } else
#endif
    {
    ...
    if (nids_params.multiproc) {
        ...
        if (qitem && (qitem->data = malloc(hdr->caplen - nids_linkoffset))) {
            ...
            if (g_async_queue_length_unlocked(cap_queue) >
                nids_params.queue_limit) {
                /* queue limit reached: drop packet - should we notify user via
                 * syslog?
                 */
                free(qitem->data);
                free(qitem);
...
static void gen_ip_frag_proc(u_char *data, int len) {
    ...
    if (iph->ip_p == 47) {
        struct GRE {
            ...
            unsigned char routing : 1;
            unsigned char key : 1;
            unsigned char seq_num : 1;
            unsigned char ssr : 1;
            unsigned short rcr_ctr : 3;
        ...
        int gre_len = sizeof(struct GRE) + sizeof(struct ip) + 14;
        struct GRE *gre = (struct GRE *)(data + sizeof(struct ip));
        // fprintf(stderr,
        //         "version: %d, c:%d, r:%d, key: %d, sn:%d, ssr:%d, rc:%d,
        //         flags: "
        //         "%d, ip size: %lu\n",
        //         gre->version, gre->checksum, gre->routing, gre->key,
        //         gre->seq_num, gre->ssr, gre->rcr_ctr, gre->flags,
        //         sizeof(struct ip));
        if (gre->version == 0) {
            if (gre->checksum) {
                gre_len += 4;
            ...
            if (!memcmp(((char *)gre) + 2, "\x65\x58", 2) && len > gre_len) {
                data += gre_len;
                iph = (struct ip *)data;
                len -= gre_len;
            }
...
static void process_udp(char *data) {
    ...
    // if (udph->uh_sum &&
    //     my_udp_check((void *)udph, ulen, iph->ip_src.s_addr,
    //     iph->ip_dst.s_addr))
    //   return;
...
void nids_register_udp(void(*x)) { register_callback(&udp_procs, x); }

void nids_unregister_udp(void(*x)) { unregister_callback(&udp_procs, x); }

void nids_register_ip(void(*x)) { register_callback(&ip_procs, x); }

void nids_unregister_ip(void(*x)) { unregister_callback(&ip_procs, x); }

void nids_register_ip_frag(void(*x)) { register_callback(&ip_frag_procs, x); }
...
static int open_live() {
    ...
    if (nids_params.device == NULL) {
        ...
        if (pcap_findalldevs(&alldevs, errbuf) != -1 && alldevs != NULL) {
            nids_params.device = alldevs->name;
        }
    }
    if (nids_params.device == NULL)
        return 0;
    ...
    // nids_params.pcap_timeout,
    //                            nids_errbuf)) == NULL)
    //   return 0;
    if ((desc = pcap_create(device, errbuf)) == NULL)
        return 0;
    if (pcap_set_snaplen(desc, 16384) < 0)
        return 0;
    if (pcap_set_promisc(desc, promisc) < 0)
        return 0;
    if (pcap_set_timeout(desc, nids_params.pcap_timeout) < 0)
        return 0;
    if (pcap_set_immediate_mode(desc, 1) < 0)
        return 0;
    if (pcap_activate(desc) < 0)
        return 0;

#ifdef __linux__
    if (!strcmp(device, "any") && nids_params.promisc && !set_all_promisc()) {
        nids_errbuf[0] = 0;
...
#define START_CAP_QUEUE_PROCESS_THREAD()                                       \
    if (nids_params.multiproc) { /* threading... */                            \
        if (!(g_thread_new("Queue process thread",                             \
                           (GThreadFunc)cap_queue_process_thread, NULL))) {    \
            strcpy(nids_errbuf, "thread: ");                                   \
...
int nids_init() {
    ...
    if (nids_params.pcap_desc)
        ...
        if ((desc = pcap_open_offline(nids_params.filename, nids_errbuf)) ==
            NULL)
            return 0;
    } else if (!open_live())
        return 0;
...
int nids_next() {
    ...
    /* threading is quite useless (harmful) in this case - should we do an API
     * change?  */
...
int nids_dispatch(int cnt) {
    ...
    START_CAP_QUEUE_PROCESS_THREAD(); /* threading... */
    if ((r = pcap_dispatch(desc, cnt, (pcap_handler)nids_pcap_handler, NULL)) ==
        -1) {
